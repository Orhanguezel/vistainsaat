// =============================================================
// FILE: src/modules/gallery/repository.ts
// =============================================================
...
/* ---- list ---- */
export async function listGalleries(params: GalleryListParams) {
  ...
  /* Inline scalar: cover image URL — prefer galleries.cover_image, fallback to first gallery_image */
  const coverImageUrl = sql<string>`COALESCE(
  ...
  const rows = await db
    .select({
      ...
      // cover + count
      cover_image: galleries.cover_image,
...
/* ---- get by slug ---- */
export async function getGalleryBySlug(slug: string, locale: string) {
  const [gallery] = await db
    ...
    .where(
      and(
        eq(galleryI18n.slug, slug),
        eq(galleryI18n.locale, locale),
  ...
  // images
  const images = await getGalleryImages(gallery.id, locale);

  return { ...gallery, images };
}

/* ---- get by id ---- */
export async function getGalleryById(id: string, locale: string) {
  const [gallery] = await db
    .select({
      id: galleries.id,
      module_key: galleries.module_key,
      ...
      display_order: galleries.display_order,
      cover_image: galleries.cover_image,
      cover_asset_id: galleries.cover_asset_id,
      created_at: galleries.created_at,
      title: galleryI18n.title,
      slug: galleryI18n.slug,
      description: galleryI18n.description,
    ...
    .leftJoin(
      galleryI18n,
      and(eq(galleries.id, galleryI18n.gallery_id), eq(galleryI18n.locale, locale)),
    )
...
/* ---- gallery images ---- */
export async function getGalleryImages(galleryId: string, locale: string) {
  return db
    .select({
      id: galleryImages.id,
    ...
    .leftJoin(
      galleryImageI18n,
      and(
        eq(galleryImages.id, galleryImageI18n.image_id),
        eq(galleryImageI18n.locale, locale),
      ),
    )
